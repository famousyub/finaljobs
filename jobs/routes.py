...
# setup variables
app_bp = Blueprint('resume',__name__,template_folder='templates',static_folder='static')
...
# url schema
@app_bp.route('/resume', methods=['GET', 'POST'])
...
def submit():
    ...
    if form.validate_on_submit():
        ...
        if file.filename.rsplit('.', 1)[1].lower() == 'pdf':
            ...
            filePath = os.path.join(os.path.abspath(os.path.dirname(__file__)),'./static/files',secure_filename(file.filename))
            file.save(filePath)
...
@app_bp.route('/review', methods=['GET', 'POST'])
def review():
